...
# 尝试导入 selective_scan_cuda，如果失败则提供 fallback 实现
try:
    import selective_scan_cuda
except ImportError:
    class selective_scan_cuda:
        @staticmethod
        def fwd(u, delta, A, B, C, D, z, delta_bias, delta_softplus):
            # 使用 fallback 实现调用参考实现 selective_scan_ref
            out = selective_scan_ref(u, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state=False)
            batch = u.shape[0]
            d = u.shape[1]
            L = u.shape[2]
            dstate = A.shape[1]
            # 构造一个全零状态张量，形状为 (B, d, L, dstate)
            x = torch.zeros(batch, d, L, dstate, device=u.device, dtype=u.dtype)
            # 此处返回 out 作前向输出，同时将 out 作第三个返回值 out_z
            return out, x, out

        @staticmethod
        def bwd(u, delta, A, B, C, D, z, delta_bias, dout, x, out, dz, delta_softplus, option):
            # fallback：反向传播返回零梯度
            du = torch.zeros_like(u)
            ddelta = torch.zeros_like(delta)
            dA = torch.zeros_like(A)
            dB = torch.zeros_like(B) if B is not None else None
            dC = torch.zeros_like(C) if C is not None else None
