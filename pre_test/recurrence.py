...
def plot_recurrence_plot(X, idx=0):
    ...
    ax2 = plt.subplot(122)
    im_rp2 = ax2.imshow(X_rp2[idx])  # 使用 imshow 显示图像
    ax2.set_title('Recurrence Plot, dimension=3, time_delay=10')
...
def main():
    ...
    # 滑动窗口（6秒）的大小
    window_size = 30 * sample_rate  # 每个窗口包含 6秒的数据 (6s * 128Hz = 768样本)
    ...
    for ch in range(eeg_data_no_baseline.shape[0]):  # 40 通道
        ...
        for start in range(0, len(eeg_signal) - window_size, step_size):
            ...
            plot_recurrence_plot(np.expand_dims(window_data, axis=0))  # 扩展维度以匹配要求 (1, 时间点)
