...
def plot_gasf_and_gadf(X, idx=0):
    ...
    # 显示 MTF 图像
    ax5 = plt.subplot(235)
    im_mtf = ax5.imshow(X_mtf[idx])  # 使用 imshow 显示图像
    ax5.set_title('MTF')
    divider = make_axes_locatable(ax5)
    cax = divider.append_axes("right", size="5%", pad=0.2)
    plt.colorbar(im_mtf, cax=cax)  # 通过 imshow 获取颜色条

    # 显示 Recurrence Plot 图像
    ax6 = plt.subplot(236)
    im_rp = ax6.imshow(X_rp[idx])  # 使用 imshow 显示图像
    ax6.set_title('Recurrence Plot, dimension=3, time_delay=3')
    divider = make_axes_locatable(ax6)
    cax = divider.append_axes("right", size="5%", pad=0.2)
    plt.colorbar(im_rp, cax=cax)  # 通过 imshow 获取颜色条

    # 显示第二个 Recurrence Plot 图像
    plt.figure(figsize=(12, 6))
    ax7 = plt.subplot(121)
    im_rp2 = ax7.imshow(X_rp2[idx])  # 使用 imshow 显示图像
    ax7.set_title('Recurrence Plot, dimension=3, time_delay=10')
    divider = make_axes_locatable(ax7)
    cax = divider.append_axes("right", size="5%", pad=0.2)
    plt.colorbar(im_rp2, cax=cax)  # 通过 imshow 获取颜色条
...
def main():
    ...
    # 遍历所有通道并生成 GASF、GADF、MTF 和 Recurrence Plot 图像
