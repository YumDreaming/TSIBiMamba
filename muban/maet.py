...
class Block(nn.Module):
    def __init__(
        ...
        self.eye_prompt = nn.Parameter(torch.zeros((1, prompt_len, dim)),
                                       requires_grad=True) if prompt is True else None
...
class MAET(nn.Module):
    def __init__(
        ...
        self.eeg_pos_embed = nn.Parameter(
            torch.zeros(1, eeg_seq_len + 1, embed_dim)) if not self.use_abs_pos_emb else None
        self.eye_pos_embed = nn.Parameter(
            torch.zeros(1, eye_seq_len + 1, embed_dim)) if not self.use_abs_pos_emb else None
    ...
    def forward_features(self, eeg, eye, alpha):
        if eeg is not None:
            ...
            eeg_cls_tokens = self.eeg_cls_token.expand(eeg.size()[0], -1,
                                                       -1)  # stole cls_tokens impl from Phil Wang, thanks
        if eye is not None:
            eye = self.eye_transform(eye)
            eye_cls_tokens = self.eye_cls_token.expand(eye.size()[0], -1,
                                                       -1)  # stole cls_tokens impl from Phil Wang, thanks
        ...
        if eye is None:
            ...
            modality_type = None
            x = x + torch.cat([self.eeg_type_embed.expand(x.size()[0], self.eeg_seq_len + 1, -1),
                               self.eye_type_embed.expand(x.size()[0], self.eye_seq_len + 1, -1)], dim=1)
            pos_embed = torch.cat(
                [self.eeg_pos_embed.expand(x.size()[0], -1, -1), self.eye_pos_embed.expand(x.size()[0], -1, -1)], dim=1)
            x = x + pos_embed
        x = self.pos_drop(x)
        ...
        if self.fc_norm is not None:
            ...
            if modality_type is not None:
                if self.prompt:
                    x = self.head_eeg(x[:, 1:].mean(dim=1)) if modality_type == 'eeg' else self.head_eye(
                        x[:, 1:].mean(dim=1))
                else:
                    x = self.head_eeg(x[:, 1:].mean(dim=1)) if modality_type == 'eeg' else self.head_eye(
                        x[:, 1:].mean(dim=1))
