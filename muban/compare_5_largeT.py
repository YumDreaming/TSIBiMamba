...
# SEQ_LENGTHS = [500, 1000, 2000, 2500, 3000, 4000, 5000, 7000, 10000, 13000,15000]
# NUM_CLASSES = 7
#
# # MAET 基础配置
...
#     y_top = ax.get_ylim()[1]
#
#     # 对 MAET(idx=2), Conformer(idx=3), BigTransformer(idx=4) 做虚线延伸
#     for idx in (2, 3, 4):
#         y = ys[idx]
...
#                 x0, y0 = x[i-2], y[i-2]
#                 x1, y1 = x[i-1], y[i-1]
#                 orig_slope = (y1 - y0) / (x1 - x0) if x1 != x0 else 1e-6
#                 if orig_slope <= 0:
#                     orig_slope = 1e-6
#                 x2 = x[i] if i < len(x) else x1 * 1.1
#                 # 要求斜率至少 1.5 倍，且保证 endpoint< x2
#                 slope_req1 = orig_slope * 1.5
#                 slope_req2 = (y_top - y1) / (x2 - x1)
#                 slope = max(slope_req1, slope_req2)
#                 x_end = x1 + (y_top - y1) / slope
#                 # 强制落在 (x1, x2)
#                 if not (x1 < x_end < x2):
#                     x_end = x1 + (x2 - x1) * 0.999
...
#         nhead=16, num_layers=8, dim_feedforward=4096, dropout=0.1
#     )
#
#     seq_models = [
#         ("TSIMamba", mamba),
...
#         labels=["TSIMamba","TSITransformer","MAET","Conformer","BigTransformer"],
...
def main():
    ...
    plot_five(
        ...
        xlabel="Sequence Length", ylabel="GPU Memory (GB)",
        title="GPU Memory Comparison",
        labels=["TSIBiMamba","TSIBiTransformer","MAET","Conformer","ViTransformer"],
        colors=["#4C72B0","#55A868","#C44E52","#8172B2","#8C8C8C"],
        save_path=os.path.join(out, "gpu_memory_comparison.png")
    )
