...
def main():
    ...
    # —— 3. 准备存储结构 ——
    num_sessions     = 4
    subj_sess_accs   = np.zeros((num_subj, num_sessions), dtype=float)
    subj_sess_precs  = np.zeros_like(subj_sess_accs)
    subj_sess_recs   = np.zeros_like(subj_sess_accs)
    subj_sess_f1s    = np.zeros_like(subj_sess_accs)

    # —— 4. 按被试×会话 循环训练/测试 ——
    for subj in range(num_subj):
        print(f"\n===== 被试 {subj+1}/{num_subj} =====")
        idxs_subj   = np.where(groups == subj)[0]
        session_len = len(idxs_subj) // num_sessions

        for sess in range(num_sessions):
            start     = sess * session_len
            end       = (sess+1)*session_len if sess < num_sessions-1 else len(idxs_subj)
            idxs_sess = idxs_subj[start:end]

            Xg_sess = all_g[idxs_sess]; Xr_sess = all_r[idxs_sess]
            y_sess  = all_y[idxs_sess].numpy()
            train_idx, test_idx = train_test_split(
                np.arange(len(y_sess)), test_size=0.1, stratify=y_sess, random_state=SEED
            )
            Xg_tr, Xg_te = Xg_sess[train_idx], Xg_sess[test_idx]
            Xr_tr, Xr_te = Xr_sess[train_idx], Xr_sess[test_idx]
            y_tr = torch.from_numpy(y_sess[train_idx]).long()
            y_te = torch.from_numpy(y_sess[test_idx]).long()
            ...
            for epoch in range(1, EPOCHS+1):
                loop = tqdm(
                    train_loader,
                    desc=f"Subj{subj+1} Sess{sess+1} Ep{epoch}/{EPOCHS}",
                    leave=False
                )
                model.train()
            ...
            model.eval()
            y_true, y_pred = [], []
            for g_b, r_b, y_b in tqdm(
                test_loader,
                desc=f"Subj{subj+1} Sess{sess+1} Testing",
                leave=False
            ):
            ...
            subj_sess_accs[subj, sess]  = acc
            subj_sess_precs[subj, sess] = prec
            subj_sess_recs[subj, sess]  = rec
            subj_sess_f1s[subj, sess]   = f1
            ...
            # 保存模型
            # sess_dir = os.path.join(save_dir, f"subject_{subj+1}_sess{sess+1}")
            # ensure_dir(sess_dir)
            # torch.save(model.state_dict(), os.path.join(sess_dir, "model.pth"))

    # —— 5. 汇总各会话指标 ——
    print("\n=== 各会话汇总（均值 ± 标准差） ===")
    metrics = {
        "Acc": subj_sess_accs,
        "Prec": subj_sess_precs,
        "Rec": subj_sess_recs,
        "F1": subj_sess_f1s
    }
    for name, arr in metrics.items():
        ...
        print(f"\n{name}:")
        for sess in range(num_sessions):
            print(f"  会话{sess+1}: {means[sess]:.2f}% ± {stds[sess]:.2f}%")

    # 打印被试×会话的完整表格
    header = "Subject\t" + "\t".join(
        f"S{sess+1}_Acc\tS{sess+1}_Prec\tS{sess+1}_Rec\tS{sess+1}_F1"
        for sess in range(num_sessions)
    )
    print("\n" + header)
    for subj in range(num_subj):
        ...
        for sess in range(num_sessions):
            row += [
                f"{subj_sess_accs[subj, sess]:.2f}%",
                f"{subj_sess_precs[subj, sess]:.2f}%",
                f"{subj_sess_recs[subj, sess]:.2f}%",
                f"{subj_sess_f1s[subj, sess]:.2f}%"
            ]
        print(f"{subj+1}\t" + "\t".join(row))
