...
class CoMambaVisionLayer(nn.Module):
    ...
    def forward(self, g, r):
        ...
        g_wins = g.permute(0, 2, 1, 3, 4)
        r_wins = r.permute(0, 2, 1, 3, 4)

        g_wins = g_wins.reshape(B, C, T*H*W)
        r_wins = r_wins.reshape(B, C, T*H*W)

        g_wins = g_wins.permute(0, 2, 1).contiguous()
        r_wins = r_wins.permute(0, 2, 1).contiguous()
        ...
        g = g_wins.permute(0, 2, 1)
        r = r_wins.permute(0, 2, 1)

        g = g.reshape(B, C, T, H, W)
        r = r.reshape(B, C, T, H, W)

        g = g.permute(0, 2, 1, 3, 4)
        r = r.permute(0, 2, 1, 3, 4)
...
class MVDataset(Dataset):
    """加载预处理后的 Heat 与 RP，输出 (T, C, H, W)"""
    def __init__(self, pre_root, subjects, transform=None):
        self.entries   = []  # list of (heat_path, rp_path, label)
        self.transform = transform
        for subj in subjects:
            subj_heat = os.path.join(pre_root, "Heat", subj)
            with open(os.path.join(subj_heat, f"{subj}_labels.txt")) as f:
                for line in f:
                    fname, lbl = line.strip().split(",")
                    heat = os.path.join(subj_heat, fname)
                    rp   = heat.replace("/Heat/", "/RP/").replace("_Heat", "_RP")
                    self.entries.append((heat, rp, int(lbl)))
    ...
    def __getitem__(self, idx):
        ...
        r = np.load(rp_path)
        g = torch.from_numpy(g).float()  # Tensor (5,9,128,128)
        r = torch.from_numpy(r).float()
        if self.transform:
            C, T, H, W = g.shape
            g = g.permute(1,0,2,3).reshape(T*C, H, W)
            r = r.permute(1,0,2,3).reshape(T*C, H, W)
            g = self.transform(g).reshape(T, C, H, W).permute(1,0,2,3)
            r = self.transform(r).reshape(T, C, H, W).permute(1,0,2,3)
        return g, r, torch.tensor(label, dtype=torch.long)

# ------------------------------ #
# 3. 训练/验证循环
# ------------------------------ #
def run_one_epoch(model, loader, criterion, optimizer, device, scaler, train=True):
    mode = "训练" if train else "验证"
    model.train() if train else model.eval()
    total_loss = correct = total = 0
    loop = tqdm(loader, desc=mode, leave=False)
    for g, r, y in loop:
        g, r, y = g.to(device), r.to(device), y.to(device)
...
# ------------------------------ #
def main(args):
    # 4.1 读取 split.json，只取 subjects 列表
    ...
    # 4.3 五折交叉验证
    all_results = []
    for subj in tqdm(subjects, desc="被试"):
        # 无增强数据集，用于获取总样本数
        base_ds = MVDataset(PRE_ROOT, [subj], transform=None)
        n = len(base_ds)
        indices = list(range(n))
        kf = KFold(n_splits=5, shuffle=True, random_state=42)
        splits = list(kf.split(indices))

        for fold, (train_idx, val_idx) in enumerate(tqdm(splits, desc=f"{subj} 折"), start=1):
            # 子集数据集
            train_ds = Subset(MVDataset(PRE_ROOT, [subj], transform=train_aug), train_idx)
            val_ds   = Subset(MVDataset(PRE_ROOT, [subj], transform=val_aug),   val_idx)
            ...
            for epoch in range(1, total_epochs + 1):
                print(f"[{subj}][Fold {fold}] Epoch {epoch}/{total_epochs} lr={scheduler.get_last_lr()[0]:.2e}")
                ...
                print(f"  训练 Acc {tr_acc*100:.2f}% | 验证 Acc {val_acc*100:.2f}%")
            ...
            print(f"==> {subj} Fold {fold} 最佳验证 Acc: {best_val_acc*100:.2f}%\n")

    # 汇总结果
    avg_acc = np.mean([r["best_val_acc"] for r in all_results])
    print(f"所有被试所有折平均最佳验证 Acc: {avg_acc*100:.2f}%")

# ------------------------------ #
# 5. CLI
# ------------------------------ #
if __name__ == "__main__":
    parser = argparse.ArgumentParser("BiMambaVision-DEAP 被试内五折交叉验证")
    parser.add_argument("--batch_size", type=int, default=40)
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--lr", type=float, default=1e-5)
    parser.add_argument('--depths', nargs='+', type=int, default=[1,1,3,2])
    parser.add_argument('--num_heads', nargs='+', type=int, default=[2,4,8,8])
    ...
    parser.add_argument('--layer_scale', type=float, default=1e-5)
    parser.add_argument('--layer_scale_conv', type=float, default=1e-5)
