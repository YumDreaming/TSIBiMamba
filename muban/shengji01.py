    ...
    def __init__(self,
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_chans, in_dim, kernel_size=(3, 3, 3),
                      stride=(2, 2, 2), padding=(1, 1, 1), bias=False),
...
class MVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # —— 如果提供了 transform，则做空间增强 ——
        C, T, H, W = g.shape
        # 1) collapse channel+time → 做成“伪多通道”图像 (T*C, H, W)
        g_img = g.permute(1, 0, 2, 3).reshape(T * C, H, W)
        r_img = r.permute(1, 0, 2, 3).reshape(T * C, H, W)

        # 2) apply spatial transforms
        g_img = self.transform(g_img)  # now (T*C, H', W')
        r_img = self.transform(r_img)

        # 3) restore shape → (T, C, H', W') → (C, T, H', W')
        H2, W2 = g_img.shape[1], g_img.shape[2]
...
def main(args):
    ...
    # )
    # —— 我们用两个 dataset：一个带 train_aug，一个带 val_aug ——
    full = MVDataset(args.data_root)  # 只是为了取 length & 划分 indices
    indices = list(range(len(dataset)))
    subjects = [os.path.basename(os.path.dirname(gasf_path)) for gasf_path, _, _ in dataset.entries]
    gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    train_idx, val_idx = next(gss.split(indices, groups=subjects))

    # train_set = torch.utils.data.Subset(dataset, train_idx)
    # val_set = torch.utils.data.Subset(dataset, val_idx)
    train_ds = MVDataset(args.data_root, transform=train_aug)
    val_ds = MVDataset(args.data_root, transform=val_aug)
    ...
    def lr_lambda(cur_epoch):
        if cur_epoch < warmup_epochs:
            ...
            progress = float(cur_epoch - warmup_epochs) / float(total_epochs - warmup_epochs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_chans', type=int, default=5)
    parser.add_argument('--depths', nargs='+', type=int, default=[1, 2, 4, 2])
