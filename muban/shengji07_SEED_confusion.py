...
def prepare_all_data(DE_dir: str, PSD_dir: str, T: int):
    """
    加载 SEED DE/PSD 分支 3D 滑窗数据并展平：
      - all_g: FloatTensor (N_total, T, C, H, W)
      - all_r: FloatTensor (N_total, T, C, H, W)
      - all_y: LongTensor  (N_total,)
      - groups: ndarray   (N_total,) 每样本所属“会话”ID
    """
...
def main():
    ...
    for sess in range(num_sess-1):
        print(f"\n===== 会话 {sess+1}/{num_sess} =====")
        idxs   = np.where(groups == sess)[0]
        Xg_sub = all_g[idxs]; Xr_sub = all_r[idxs]
        y_sub  = all_y[idxs].numpy()
        assert y_sub.min() >= 0 and y_sub.max() < NUM_CLASSES

        # 9:1 划分
        inds = np.arange(len(y_sub))
        tr_idx, te_idx = train_test_split(inds, test_size=0.1, stratify=y_sub, random_state=SEED)
        ...
        for epoch in range(1, EPOCHS+1):
            loop = tqdm(train_loader, desc=f"S{sess+1}-Ep{epoch}/{EPOCHS}", leave=False)
        ...
        # 测试并收集预测
        model.eval()
        y_true_sess, y_pred_sess = [], []
        with torch.no_grad():
            for g_b, r_b, y_b in test_loader:
                g_b, r_b, y_b = g_b.to(DEVICE), r_b.to(DEVICE), y_b.to(DEVICE)
                preds = model(g_b, r_b).argmax(dim=1)
                y_true_sess.extend(y_b.cpu().numpy())
                y_pred_sess.extend(preds.cpu().numpy())

        # 计算并打印会话准确率
        acc = (np.array(y_true_sess) == np.array(y_pred_sess)).mean() * 100
        print(f"会话{sess+1} 9:1 准确率: {acc:.2f}%")
        sess_accs.append(acc)

        # 累积全局结果
        y_true_global.extend(y_true_sess)
        y_pred_global.extend(y_pred_sess)

    # 4. 汇总所有会话准确率
    overall_mean = np.mean(sess_accs)
    overall_std  = np.std(sess_accs)
    print(f"\n=== 全部会话 平均准确率: {overall_mean:.2f}%  Std: {overall_std:.2f}% ===")
    ...
    plt.tight_layout()
    fig_path = os.path.join(save_dir, 'confusion_matrix_global.png')
    plt.savefig(fig_path, dpi=200)
    print(f"全局混淆矩阵可视化已保存至: {fig_path}")
