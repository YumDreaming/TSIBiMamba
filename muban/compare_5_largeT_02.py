...
def main():
    ...
    # 打印每个模型在每个序列长度上的显存占用
    model_names = [n for n, _ in seq_models]
    names_with_maet = model_names[:2] + ["MAET"] + model_names[2:]
    print("\n=== GPU Memory (GB) per Sequence Length ===")
    print("Sequence Lengths:", SEQ_LENGTHS)
    for name, mem in zip(names_with_maet, mem_lists):
        formatted = ", ".join(f"{v:.3f}" if not math.isnan(v) else "nan" for v in mem)
    ...
    print("Sequence Lengths:", SEQ_LENGTHS)
    for name, sp in zip(names_with_maet, sp_lists):
        formatted = ", ".join(f"{v:.4f}" if not math.isnan(v) else "nan" for v in sp)
        print(f"{name:15s}: {formatted}")
