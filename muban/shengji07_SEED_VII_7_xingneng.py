...
def main():
    ...
    NS       = int(N_total / num_subj)
    print(f"共计 {num_subj} 位被试，每位被试样本数 = {NS}，总样本数 = {N_total}")
    ...
    for subj in range(num_subj):
        ...
        Xg_sub = all_g[idxs]; Xr_sub = all_r[idxs]
        y_sub  = all_y[idxs].numpy()
        assert y_sub.min() >= 0 and y_sub.max() < NUM_CLASSES

        # —— 单次 9:1 划分 ——
        train_idx, test_idx = train_test_split(
            np.arange(NS), test_size=0.1, stratify=y_sub, random_state=SEED
        )

        Xg_tr, Xg_te = Xg_sub[train_idx], Xg_sub[test_idx]
        Xr_tr, Xr_te = Xr_sub[train_idx], Xr_sub[test_idx]
        y_tr = torch.from_numpy(y_sub[train_idx]).long()
        y_te = torch.from_numpy(y_sub[test_idx]).long()
        ...
        for epoch in range(1, EPOCHS+1):
            loop = tqdm(train_loader, desc=f"Subj{subj+1} Ep{epoch}/{EPOCHS}", leave=False)
        ...
        with torch.no_grad():
            for g_b, r_b, y_b in test_loader:
                g_b, r_b = g_b.to(DEVICE), r_b.to(DEVICE)
                logits = model(g_b, r_b)
                preds  = logits.argmax(dim=1).cpu().numpy()
                y_pred_subj.extend(preds)
                y_true_subj.extend(y_b.numpy())

        # 准确率
        correct   = np.sum(np.array(y_pred_subj) == np.array(y_true_subj))
        total     = len(y_true_subj)
        acc_subj  = correct / total * 100
        # Precision / Recall / F1 (macro)
        prec_subj = precision_score(y_true_subj, y_pred_subj, average='macro') * 100
        rec_subj  = recall_score(y_true_subj, y_pred_subj, average='macro') * 100
        f1_subj   = f1_score(y_true_subj, y_pred_subj, average='macro') * 100

        print(f"被试{subj+1} 准确率: {acc_subj:.2f}% ({correct}/{total})")
        print(f"被试{subj+1} Precision(macro): {prec_subj:.2f}%  Recall(macro): {rec_subj:.2f}%  F1(macro): {f1_subj:.2f}%")

        # 累计结果
        subj_accs.append(acc_subj)
        subj_precisions.append(prec_subj)
        subj_recalls.append(rec_subj)
        subj_f1s.append(f1_subj)
        ...
        # torch.save(model.state_dict(), os.path.join(model_sub_dir, "model.pth"))
        print(f"[被试{subj+1}] 模型已保存")

    # —— 所有被试指标汇总 ——
    mean_acc       = np.mean(subj_accs);    std_acc       = np.std(subj_accs)
    mean_prec      = np.mean(subj_precisions); std_prec      = np.std(subj_precisions)
    mean_rec       = np.mean(subj_recalls);    std_rec       = np.std(subj_recalls)
    mean_f1        = np.mean(subj_f1s);     std_f1        = np.std(subj_f1s)

    print(f"\n=== 所有被试平均准确率: {mean_acc:.2f}%  Std: {std_acc:.2f}% ===")
    print(f"=== 所有被试平均 Precision: {mean_prec:.2f}%  Std: {std_prec:.2f}% ===")
    print(f"=== 所有被试平均 Recall: {mean_rec:.2f}%  Std: {std_rec:.2f}% ===")
    print(f"=== 所有被试平均 F1: {mean_f1:.2f}%  Std: {std_f1:.2f}% ===")

    # —— 整体混淆矩阵 & 分类报告 ——
    cm_all = confusion_matrix(all_true, all_pred, labels=list(range(NUM_CLASSES)))
    print("所有被试汇总混淆矩阵：")
    print(cm_all)

    print("\n=== Classification Report (所有被试) ===")
    print(classification_report(all_true, all_pred, labels=list(range(NUM_CLASSES))))

    # —— 混淆矩阵可视化 ——
    import matplotlib.pyplot as plt

    cm_norm = cm_all.astype(float) / cm_all.sum(axis=1)[:, None] * 100
