...
def main():
    ...
    Xg_sub, Xr_sub = all_g[idxs], all_r[idxs]
    y_sub = all_y[idxs].numpy()

    skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=SEED)
    fold_accs, y_true_all, y_pred_all = [], [], []

    for fold, (tr_idx, te_idx) in enumerate(skf.split(np.zeros_like(y_sub), y_sub), start=1):
        print(f"\n--- 被试{subj+1} Fold {fold}/10 ---")

        # 划分并加载 DataLoader
        y_tr = torch.from_numpy(y_sub[tr_idx]).long()
        y_te = torch.from_numpy(y_sub[te_idx]).long()
        train_loader = DataLoader(TensorDataset(Xg_sub[tr_idx], Xr_sub[tr_idx], y_tr),
                                  batch_size=BATCH_SIZE, shuffle=True,
                                  num_workers=4, pin_memory=True)
        test_loader  = DataLoader(TensorDataset(Xg_sub[te_idx], Xr_sub[te_idx], y_te),
        ...
        for epoch in range(1, EPOCHS+1):
            ...
            loop = tqdm(train_loader, desc=f"Ep{epoch}/{EPOCHS}", leave=False)
            for g_b, r_b, y_b in loop:
                g_b, r_b, y_b = g_b.to(DEVICE), r_b.to(DEVICE), y_b.to(DEVICE)
                optimizer.zero_grad()
                outputs = model(g_b, r_b)
        ...
        ax1.plot(epochs, train_losses, color='#4C72B0', label='loss')
        ax2.plot(epochs, train_accs,   color='#55A868', label='accuracy')
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('training loss',    color='#4C72B0')
        ax2.set_ylabel('training accuracy', color='#55A868')
        ...
        ax1.legend(
            ...
            loc='upper right',
            bbox_to_anchor=(1.0, 0.90)   # y=0.98，相比原来的(1,1)稍微下移2%
        )
        fig.tight_layout()

        # plt.title(f"Subject{subj+1} Fold{fold} Training Curve")
        curve_path = os.path.join(
            save_dir,
            f"subject_{subj+1}_fold{fold}_train_curve.png"
        )
        ...
        model.eval()
        corr, tot = 0, 0
        class_corr = np.zeros(NUM_CLASSES, int)
        class_tot  = np.zeros(NUM_CLASSES, int)
        with torch.no_grad():
            for g_b, r_b, y_b in test_loader:
                g_b, r_b, y_b = g_b.to(DEVICE), r_b.to(DEVICE), y_b.to(DEVICE)
                preds = model(g_b, r_b).argmax(dim=1)
                corr  += (preds==y_b).sum().item(); tot += y_b.size(0)
                for i in range(y_b.size(0)):
                    lbl = y_b[i].item()
                    class_tot[lbl]  += 1
                    class_corr[lbl] += int(preds[i]==y_b[i])
                y_true_all.extend(y_b.cpu().tolist())
                y_pred_all.extend(preds.cpu().tolist())

        acc = corr/tot*100
        fold_accs.append(acc)
        print(f"Fold{fold} 准确率: {acc:.2f}%")
        for cls in range(NUM_CLASSES):
            acc_c = (class_corr[cls]/class_tot[cls]*100) if class_tot[cls] else 0.0
            print(f"  类别{cls}: {acc_c:.2f}% ({class_corr[cls]}/{class_tot[cls]})")

        subj_dir = os.path.join(save_dir, f"subject_{subj+1}")
        ensure_dir(subj_dir)
        torch.save(model.state_dict(), os.path.join(subj_dir, f"fold{fold}.pth"))
    ...
    plt.title("Subject 4 - Confusion Matrix (10-fold)")
    cm_path = os.path.join(save_dir, "subject_4_confmat.png")
