...
def main():
    ...
    NUM_CHAN    = int(cfg["input"]["C"]) - 1   # 5→4
    ...
    for fold, (tr_idx, te_idx) in enumerate(skf.split(np.zeros(NS), y_sub), start=1):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            loop = tqdm(train_loader,
                        desc=f"S{subj+1} F{fold} Ep{epoch}/{EPOCHS}", leave=False)
            for g_b, r_b, y_b in loop:
                ...
                batch_correct = (preds == y_b).sum().item()
                ...
                loop.set_postfix(
                    loss=f"{loss.item():.4f}",
                    acc=f"{batch_correct / batch_total * 100:.2f}%"
                )

            if scheduler:
                scheduler.step()

            # 计算并记录本 epoch 的平均 loss & acc
        ...
        metrics = {"loss": train_loss_list, "acc": train_acc_list}
        torch.save(metrics, os.path.join(model_dir, f"fold{fold}_metrics.pt"))
        print(f"  Fold{fold} 训练指标已保存到: {model_dir}/fold{fold}_metrics.pt")
        ...
        plt.legend()
        curve_path = os.path.join(model_dir, f"fold{fold}_train_curve.png")
        plt.savefig(curve_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"  Fold{fold} 训练曲线已保存到: {curve_path}")
        ...
        print(f"  Fold{fold} 测试准确率: {acc:.2f}%")
        for cls in range(NUM_CLASSES):
            acc_c = (class_correct[cls] / class_total[cls] * 100
                     if class_total[cls] else 0.0)
            print(f"    类别{cls}: {acc_c:.2f}% ({class_correct[cls]}/{class_total[cls]})")
    ...
    cmn = cmn / cmn.sum(1, keepdims=True) * 100
    ...
    fig_path = os.path.join(save_dir, "subject_4_confmat.png")
    plt.savefig(fig_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"混淆矩阵已保存到: {fig_path}")
