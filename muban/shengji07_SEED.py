    ...
    for sess in range(num_sess):
        ...
        for fold, (tr_idx, te_idx) in enumerate(skf.split(np.zeros(NS), y_sub), start=1):
            ...
            if use_wandb:
                import wandb
                wandb.init(
                    project="SEED-3Classes",
                    name=f"sess{sess+1}_fold{fold}",
            ...
            for epoch in range(1, EPOCHS+1):
                loop = tqdm(train_loader,
                            desc=f"S{sess+1}-F{fold}-Ep{epoch}/{EPOCHS}",
                            leave=False)
                running_loss = 0.0
                for g_b, r_b, y_b in loop:
                    g_b, r_b, y_b = (
                        g_b.to(DEVICE), r_b.to(DEVICE), y_b.to(DEVICE)
                    )
                    optimizer.zero_grad()
                ...
                if use_wandb:
                    wandb.log({f"sess{sess+1}_fold{fold}/train_loss": avg_loss, "epoch": epoch})
            ...
            print(f"会话{sess+1} Fold{fold} 准确率: {overall_acc:.2f}%")
            ...
            # 7. 保存模型
            model_sub = os.path.join(save_dir, f"session_{sess+1}")
            ensure_dir(model_sub)
            model_path = os.path.join(model_sub, f"fold{fold}.pth")
            torch.save(model.state_dict(), model_path)
            print(f"[S{sess+1}F{fold}] 已保存: {model_path}")

            if use_wandb:
                wandb.log({f"sess{sess+1}_fold{fold}/test_acc": overall_acc})
                wandb.finish()

        mean_acc = np.mean(fold_accs)
        std_acc  = np.std(fold_accs)
        print(f"\n>>> 会话{sess+1} 10折平均: {mean_acc:.2f}%  Std: {std_acc:.2f}% <<<")
        sess_accs.append(mean_acc)

    # 8. 汇总所有会话
    overall_mean = np.mean(sess_accs)
    overall_std  = np.std(sess_accs)
    print(f"\n=== 全部会话 10折平均: {overall_mean:.2f}%  Std: {overall_std:.2f}% ===")
