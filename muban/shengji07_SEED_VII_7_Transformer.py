...
class CoBiTransformerLayer(nn.Module):
    ...
    def forward(self, g, r, *_):
        # Transformer 接受 (seq_len, batch, d_model)
        g2 = self.trans_g(g.transpose(0,1)).transpose(0,1)
        r2 = self.trans_r(r.transpose(0,1)).transpose(0,1)
...
class EnBiTransformerLayer(nn.Module):
    ...
    def forward(self, x, *_):
        x2 = self.trans(x.transpose(0,1)).transpose(0,1)
...
def main():
    ...
    num_subj, NS = int(groups.max())+1, int(N_total/(int(groups.max())+1))
    ...
    for subj in range(num_subj):
        ...
        for fold, (tr_idx, te_idx) in enumerate(skf.split(np.zeros(NS), y_sub), start=1):
            ...
            for epoch in range(1, EPOCHS+1):
                loop = tqdm(train_loader, desc=f"Subj{subj+1} Fold{fold} Ep{epoch}/{EPOCHS}", leave=False)
                running_loss = 0.0
            ...
            # 测试
            model.eval()
            correct_total=0; total_samples=0
            ...
            with torch.no_grad():
                for g_b, r_b, y_b in test_loader:
                    ...
                    for i in range(y_b.size(0)):
                        lbl = y_b[i].item()
                        class_total[lbl]+=1
                        if preds[i].item()==lbl: class_correct[lbl]+=1

            overall_acc = correct_total/total_samples*100
            print(f"被试{subj+1} Fold{fold} 整体准确率: {overall_acc:.2f}%")
            for cls in range(NUM_CLASSES):
                acc_cls = (class_correct[cls]/class_total[cls]*100) if class_total[cls]>0 else 0.0
            ...
            ensure_dir(sub_dir)
            torch.save(model.state_dict(), os.path.join(sub_dir, f"fold{fold}.pth"))

        mean_acc = np.mean(fold_accs); std_acc = np.std(fold_accs)
        print(f">>> 被试{subj+1} 10 折平均准确率: {mean_acc:.2f}%  Std: {std_acc:.2f}% <<<")
        subj_accs.append(mean_acc)

    overall_mean = np.mean(subj_accs); overall_std = np.std(subj_accs)
    print(f"\n=== 所有被试 10 折平均准确率: {overall_mean:.2f}%  Std: {overall_std:.2f}% ===")
