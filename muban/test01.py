...
if __name__ == '__main__':
    ...
    # 进度条共 4 步
    pbar = tqdm(total=4, desc="自测流程", ncols=80)

    # Step 1: 生成随机数据
    pbar.set_description("Step 1: 生成随机数据")
    g_arr = torch.randn(batch_size, num_windows, in_chans, resolution, resolution, device=device)
    r_arr = torch.randn_like(g_arr)
    pbar.update(1)

    # Step 2: 变形 (展平 windows 维度)
    pbar.set_description("Step 2: 变形数据为 (B*W, C, H, W)")
    g = g_arr.view(batch_size * num_windows, in_chans, resolution, resolution)
    r = r_arr.view(batch_size * num_windows, in_chans, resolution, resolution)
    pbar.update(1)

    # Step 3: 模型初始化
    pbar.set_description("Step 3: 初始化模型")
    model = BiMambaVision(
        depths=depths,
        num_heads=num_heads,
    ...
    # Step 4: 前向推理
    pbar.set_description("Step 4: 前向推理")
    with torch.no_grad():
        out = model(g, r)
    pbar.update(1)
    pbar.close()
    ...
    # 最终打印一下输入输出形状
    print(f"输入 g, r 形状: {g.shape}, {r.shape}")
