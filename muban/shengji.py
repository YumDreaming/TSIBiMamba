...
if __name__ == '__main__':
    ...
    print(device)
    batch_size = 20
    g = torch.randn(batch_size, in_chans, 9, 128, 128).to(device)  # (B, C, H, W)
    r = torch.randn(batch_size, in_chans, 9, 128, 128).to(device)  # (B, C, H, W)
    ...
    output = model(g, r)
    print(f"输出: {output.shape}")   # 输出  [2, 1024, 7, 7]