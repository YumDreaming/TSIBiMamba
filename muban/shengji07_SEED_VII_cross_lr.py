...
def load_branch_npy(branch_dir: str):
    """
    加载单个分支的滑窗数据并重排通道：
      输入: t6x_89.npy (20, NS, T, 8, 9, 5)
            t6y_89.npy (20*NS,)
      输出: X (20, NS, T, 4, 8, 9), y (20*NS,)
    """
...
def prepare_all_data(DE_dir: str, PSD_dir: str, T: int):
    """
    加载所有被试的 DE & PSD 数据，展平并返回:
      all_g, all_r: FloatTensor (N_total, T, 4, 8, 9)
      all_y:        LongTensor  (N_total,)
      groups:       ndarray     (N_total,) 每个样本的被试 ID
    """
...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(all_g, all_y, groups), start=1):
        ...
        print(f"\n--- Fold {fold}: 留出被试 {test_subj} 作为测试集 ---")
        ...
        # 使用 ReduceLROnPlateau，当 loss 停滞时自动衰减学习率
        ...
        if use_wandb:
            import wandb
            wandb.init(project="SEED-VII-cross", name=f"fold{fold}", config=cfg, reinit=True)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            loop = tqdm(
                train_loader,
                desc=f"Fold{fold} Ep{epoch}/{EPOCHS}",
                leave=False
            )
            for g_b, r_b, y_b in loop:
                ...
                # 在进度条同时显示当前 batch 的 loss 和 lr
                loop.set_postfix(loss=f"{loss.item():.4f}",
                                 lr=f"{optimizer.param_groups[0]['lr']:.2e}")

            # 计算 epoch 平均 loss，并让调度器根据它自动衰减 lr
            epoch_loss = running_loss / len(train_loader.dataset)
            scheduler.step(epoch_loss)

            # 在每个 epoch 结束后打印衰减后的 lr 和本 epoch loss
            current_lr = optimizer.param_groups[0]['lr']
            print(f"Epoch {epoch} 完成，avg_loss={epoch_loss:.4f}, new_lr={current_lr:.2e}")

            if use_wandb:
                wandb.log({f"fold{fold}/train_loss": epoch_loss,
        ...
        print(f"Fold{fold} 被试{test_subj} 测试集 整体准确率: {overall_acc:.2f}%")
    ...
    print(f"\n=== 跨被试留一验证 平均准确率: {mean_acc:.2f}%  Std: {std_acc:.2f}% ===")
