...
def main():
    ...
    for de_fname in de_files:
        ...
        g_data, g_labels = load_branch(g_path, 'arousal_labels')
        r_data, r_labels = load_branch(r_path, 'arousal_labels')
        ...
        Xg_tr, Xr_tr, y_tr = Xg[train_idx], Xr[train_idx], y[train_idx]
        Xg_te, Xr_te, y_te = Xg[test_idx], Xr[test_idx], y[test_idx]
        ...
        scheduler = (
            ...
            if scheduler_type == 'cos' else None
        )
        ...
        for epoch in range(1, E+1):
            loop = tqdm(train_loader,
                        desc=f"Subject{sid} Epoch{epoch}/{E}",
                        leave=False)
        ...
        print(f"Subject {sid} Accuracy: {acc:.2f}%")
        all_acc.append(acc)

    # 输出总体平均准确率
    mean_acc = np.mean(all_acc)
    std_acc  = np.std(all_acc)
    print(f"\n=== All Subjects — Mean Accuracy: {mean_acc:.2f}%  Std: {std_acc:.2f}% ===")
    ...
    # 计算按真实标签归一化后的百分比矩阵
    cm_percent = agg_cm.astype('float') / agg_cm.sum(axis=1)[:, None] * 100
    print("Aggregated Confusion Matrix (percentage):")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'aggregated_confusion_matrix_percent.png'))
    plt.close()
