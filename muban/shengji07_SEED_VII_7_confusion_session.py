...
def main():
    ...
    for subj in range(num_subj):
        ...
        for sess in range(num_sessions):
            ...
            for epoch in range(1, EPOCHS+1):
                for g_b, r_b, y_b in train_loader:
            ...
            total          = len(y_true)
            acc_subj_sess  = correct / total * 100
            subj_sess_accs[subj, sess] = acc_subj_sess
            print(f"  会话{sess+1} 准确率: {acc_subj_sess:.2f}% ({correct}/{total})")
    ...
    # —— 5. 汇总每会话的平均准确率 & 标准差 ——
    sess_means = subj_sess_accs.mean(axis=0)
    sess_stds  = subj_sess_accs.std(axis=0)
    print("\n=== 各会话汇总（平均准确率 ± 标准差） ===")
    for sess in range(num_sessions):
        print(f"会话{sess+1}: {sess_means[sess]:.2f}%  ± {sess_stds[sess]:.2f}%")

    # （可选）打印被试×会话矩阵
    print("\nSubject\tSess1\tSess2\tSess3\tSess4")
    for subj in range(num_subj):
        row = subj_sess_accs[subj]
        print(f"{subj+1}\t" + "\t".join(f"{val:.2f}%" for val in row))
