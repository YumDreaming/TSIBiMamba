...
class CoSSM(nn.Module):
    def __init__(self, input_size, output_sizes=[1024], dropout=0.2, activation='GELU', causal=False, mamba_config=None):
        """
        input_size：跨分支 SSM 的初始输入维度，一般等于 BaseCNN 输出（512）。
        output_sizes：列表，每个元素表示一层输出维度。如 [512] 表示一层输出 512；
                      [512,256] 表示两层，第一层 512→512，第二层 512→256。
        """
        super().__init__()
        res_list   = []
        mamba_list = []
...
class EnSSM(nn.Module):
    def __init__(self, input_size, output_sizes=[1024], activation='GELU', dropout=0.2, causal=False, mamba_config=None):
        """
        input_size：编码 SSM 的初始输入维度，一般等于拼接后跨分支输出维度（2×last_out_sz）。
        output_sizes：列表，每个元素表示一层输出维度；与 CoSSM 同理，可调整深度。
        """
        super().__init__()
        res_list   = []
        mamba_list = []
...
class BiMambaVision(nn.Module):
    def __init__(self, mm_input_size=512, mm_output_sizes=[512],
        ...
        """
        mm_input_size:  BaseCNN 输出维度，默认为 512。
        mm_output_sizes: 跨分支 SSM 的输出列表，比如 [512] 或 [512,256]，控制网络深度与宽度。
        dropout/activation/causal/mamba_config: 直接传给 CoSSM 和 EnSSM。
        """
        super().__init__()
        ...
        enc_input = mm_output_sizes[-1] * 2
        self.enssm_encoder = EnSSM(enc_input,
                                   [enc_input],  # 仅一层，输出也为 enc_input
                                   activation=activation, dropout=dropout,
                                   causal=causal, mamba_config=mamba_config)
        # 最终分类器：输入 enc_input，输出 NUM_CLASSES
        self.classifier = nn.Linear(enc_input, NUM_CLASSES)

    def forward(self, g, r, g_inference_params=None, r_inference_params=None):
        # g, r: (B, T=6, C=4, H=8, W=9)
        B, T, C, H, W = g.shape
        g_out = g.view(B*T, C, H, W)             # (B*T, 4, 8,9)
        r_out = r.view(B*T, C, H, W)
        g_cnn = self.base_cnn_g(g_out)           # (B*T, 512)
        r_cnn = self.base_cnn_r(r_out)           # (B*T, 512)
        g_feat = g_cnn.view(B, T, -1)            # (B, 6, 512)
        r_feat = r_cnn.view(B, T, -1)            # (B, 6, 512)
        g_cossm, r_cossm = self.cossm_encoder(g_feat, r_feat,
                                              g_inference_params, r_inference_params)
        x = torch.cat([g_cossm, r_cossm], dim=-1)  # (B, 6, 2×mm_output_sizes[-1])
        x = self.enssm_encoder(x)                  # (B, 6, enc_input)
        last = x[:, -1, :]                         # (B, enc_input)
...
def main():
    # —— 1. 载入配置 ——
    cfg = load_config("config_VII_7_array.yaml")
    ...
    for subj in range(num_subj):
        ...
        for fold, (tr_idx, te_idx) in enumerate(skf.split(np.zeros(NS), y_sub), start=1):
            ...
            model = BiMambaVision(
                ...
                mm_output_sizes = cfg["mmmamba"]["mm_output_sizes"],  # 列表形式，如 [512] 或 [512,256]
