...
# -*- coding: utf-8 -*-
"""
SEED-VII 单分支 DE 数据训练脚本（单被试内 9:1 划分，含学习率调度与 per-class 准确率输出）
仅加载 DE 分支，单分支 Mamba 处理（bidirectional=False）。
"""
...
class BaseCNN(nn.Module):
    def __init__(self, in_chan):
        ...
        # 此时全局 IMG_ROWS/IMG_COLS 已在 main() 覆盖
...
class EnResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return out + res  # (B, output_size, seq_len)
...
class EnBiMambaLayer(nn.Module):
    def __init__(self, d_model, dropout, mamba_config):
        ...
        # 单分支，移除 bidirectional 字段再传给 Mamba
    ...
    def forward(self, x, inference_params=None):
        ...
        return x + self.norm(out)  # (B, seq_len, d_model)
...
class MambaVision(nn.Module):
    ...
    def forward(self, g, inference_params=None):
        # g: (B, T, C, H, W)
        B, T, C, H, W = g.shape
        x = g.view(B*T, C, H, W)
        feat = self.base_cnn(x)        # (B*T, mm_input_size)
        feat = feat.view(B, T, -1)     # (B, T, mm_input_size)
        enc  = self.encoder(feat, inference_params)
        last = enc[:, -1, :]           # (B, mm_output_sizes[-1])
...
def load_branch_npy(branch_dir: str):
    ...
    # 转置通道维度并去掉第一个通道 → (N, T, 4, H, W)
    X = X.transpose(0,1,2,5,3,4)[..., 1:, :, :].copy()
    return X, y

def prepare_de_data(DE_dir: str, T: int):
    X_de, y_de = load_branch_npy(DE_dir)
    num_subj, NS = X_de.shape[0], X_de.shape[1]
    flat = X_de.reshape((-1, T, X_de.shape[3], IMG_ROWS, IMG_COLS))
    groups = np.repeat(np.arange(num_subj), NS)
    return torch.from_numpy(flat).float(), torch.from_numpy(y_de).long(), groups


# -------------------- 主训练流程 --------------------

def main():
    cfg = load_config("only_DE.yaml")
    ...
    # —— 加载 DE 数据 ——
    DE_dir = os.path.join(cfg["data_dir"], "DE")
    print("加载 DE 分支数据…")
    all_g, all_y, groups = prepare_de_data(DE_dir, SEQ_LEN)
    ...
    for subj in range(num_subj):
        ...
        train_loader = DataLoader(
            TensorDataset(Xg_tr, y_tr),
            batch_size=batch_size, shuffle=True,
            num_workers=4, pin_memory=True
        )
        test_loader  = DataLoader(
            TensorDataset(Xg_te, y_te),
        ...
        for epoch in range(1, epochs+1):
            ...
            for g_b, y_b in loop:
                g_b, y_b = g_b.to(device), y_b.to(device)
                optimizer.zero_grad()
                loss = criterion(model(g_b), y_b)
                loss.backward()
                optimizer.step()
        ...
        y_true, y_pred = [], []
        with torch.no_grad():
            for g_b, y_b in test_loader:
                g_b = g_b.to(device)
                preds = model(g_b).argmax(dim=1).cpu().numpy()
                y_pred.extend(preds)
