...
for y, lab, c in zip(mem_lists, model_names, colors):
    ax.plot(SEQ_LENGTHS, y, marker='o', markersize=4, linewidth=1.2, color=c, label=lab)
# 外推虚线
y_top = ax.get_ylim()[1]
for idx in (2, 3, 4):
    y = mem_lists[idx]
    for i, v in enumerate(y):
        if math.isnan(v) and i >= 2:
            ...
            ax.plot([x1, x_end], [y1, y_top], linestyle='--', linewidth=1.0, color=colors[idx])
            break
ax.set_xlabel("Sequence Length", fontsize=14, fontweight='bold')
ax.set_ylabel("GPU Memory (GB)", fontsize=14, fontweight='bold')
ax.tick_params(axis='both', labelsize=10)
# 压缩图例框
ax.legend(loc='upper left', fontsize=12, frameon=True,
          handlelength=1.0, handletextpad=0.3, columnspacing=0.3, borderpad=0.2)
fig.tight_layout()
fig.savefig("C:/Users\Yum\Desktop\cpp\pic01\memory_plot_bold.pdf", format='pdf', bbox_inches='tight')
...
for idx in (2, 3, 4):
    ...
    for i, v in enumerate(y):
        if math.isnan(v) and i >= 2:
            ...
            break
ax.set_xlabel("Sequence Length", fontsize=14, fontweight='bold')
ax.set_ylabel("Inference Time (s)", fontsize=14, fontweight='bold')
ax.tick_params(axis='both', labelsize=10)
ax.legend(loc='upper left', fontsize=12, frameon=True,
          handlelength=1.0, handletextpad=0.3, columnspacing=0.3, borderpad=0.2)
fig.tight_layout()
fig.savefig("C:/Users\Yum\Desktop\cpp\pic01\speed_plot_bold.pdf", format='pdf', bbox_inches='tight')
