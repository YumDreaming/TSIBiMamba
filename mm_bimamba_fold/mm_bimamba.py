...
from models.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj
...
class Mamba(nn.Module):
    def __init__(
        ...
        # 软加法激活： softplus(x) = log(1 + e^x)
        # 反 softplus： 令 y = softplus(x)，则 x = y + log(-expm1(-y))
    ...
    def step(self, a_hidden_states, a_conv_state, a_ssm_state, v_hidden_states, v_conv_state, v_ssm_state):
        ...
        if selective_state_update is None:
            ...
            v_y = selective_state_update(
                v_ssm_state, v_x, v_dt, A, v_B, v_C, self.a_D, z=v_z, dt_bias=self.v_dt_proj.bias, dt_softplus=True
            )
